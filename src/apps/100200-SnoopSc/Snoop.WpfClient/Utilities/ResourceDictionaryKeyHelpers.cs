namespace Snoop.WpfClient.Utilities
{
    ...
    public static class ResourceDictionaryKeyHelpersNew
    {
            ...
            while (dependencyObject is Visual or Visual3D)
            {
                if (dependencyObject is FrameworkElement fe)
                {
                    var resourceKey = GetKeyInResourceDictionary(fe.Resources, resourceItem)
                                      ?? GetKeyInResourceDictionary(FrameworkElementHelperNew.GetStyle(fe)?.Resources, resourceItem)
                                      ?? GetKeyInResourceDictionary(FrameworkElementHelperNew.GetTemplate(fe)?.Resources, resourceItem)
                                      ?? GetKeyInResourceDictionary(FrameworkElementHelperNew.GetThemeStyle(fe)?.Resources, resourceItem);
                ...
                else if (dependencyObject is FrameworkContentElement fce)
                {
                    var resourceKey = GetKeyInResourceDictionary(fce.Resources, resourceItem)
                                      ?? GetKeyInResourceDictionary(FrameworkElementHelperNew.GetStyle(fce)?.Resources, resourceItem)
                                      ?? GetKeyInResourceDictionary(FrameworkElementHelperNew.GetThemeStyle(fce)?.Resources, resourceItem);
                    if (resourceKey is not null)
                    {
                        return resourceKey;
                    }
        ...
        {
            foreach (var cacheEntry in SystemResourcesCacheNew.Instance.SystemResources.Reverse())
            {
                {
        ...
        public static object? GetKeyInResourceDictionary(ResourceDictionary? dictionary, object? resourceItem)
        {
            ...
            if (baseDictionaryFieldInfo?.GetValue(dictionary) is Hashtable hashtable
                ...
                && UtilsNew.IgnoreErrors(() => hashtable.ContainsValue(resourceItem), true) is false)
            {
