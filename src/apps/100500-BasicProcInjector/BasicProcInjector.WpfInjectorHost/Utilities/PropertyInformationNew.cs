namespace BasicProcInjector.WpfInjectorHost.Utilities
{
    ...
    public class PropertyInformationNew : DependencyObject, IComparable, INotifyPropertyChanged
    {
        ...
        private PropertyFilterNew? filter;
        ...
        public PropertyInformationNew(object target, PropertyDescriptor? property, string propertyName, string propertyDisplayName)
        {
            this.Target = target;
            ...
            if (property is not null)
            {
                ...
                if (dp is not null)
                {
                    ...
                    {
                        binding.Converter = NullStyleConverterNew.DefaultInstance;
        ...
        /// <param name="propertyDisplayName">the display name for the property that goes in the name column</param>
        public PropertyInformationNew(object target, PropertyDescriptor? property, DependencyProperty? dependencyProperty, BindingBase binding, string propertyDisplayName)
        {
            this.Target = target;
            this.property = property;
        ...
        public PropertyInformationNew(object target, object? component, string displayName, object? value, bool isCopyable = false)
        ...
        public static readonly DependencyProperty ValueProperty =
            DependencyProperty.Register(
                nameof(Value),
                typeof(object),
                typeof(PropertyInformationNew),
                new PropertyMetadata(OnValueChanged));

        private static void OnValueChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ((PropertyInformationNew)d).OnValueChanged(e);
        }
...
#pragma warning restore WPF0036 // Avoid side effects in CLR accessors.
                ...
                try
                {
                    this.property.SetValue(this.Target, StringValueConverterNew.ConvertFromString(targetType, value));
                }
                catch
                {
                }
                    ...
                    default:
                        {
                            if (this.Target is DependencyObject dependencyObject)
                            {
                                ...
                                {
                                    var resourceKey = ResourceKeyCacheNew.Instance.GetOrAddKey(dependencyObject, value);
        ...
        public string DescriptiveValue
        {
            get
            {
                ...
                if (stringValueIsTypeToString)
                {
                    switch (value)
                    {
                        ...
                        case PropertyPath propertyPath:
                            return $"[PropertyPath] {XamlWriterHelperNew.GetXamlAsXElement(propertyPath).Value}";
                    }
...

                {
                    ...
                    if (ResourceKeyHelperNew.IsValidResourceKey(this.ResourceKey))
                    {
                    ...
                    {
                        var bindingDescriptiveString = BindingDisplayHelperNew.BuildBindingDescriptiveString(binding);
        ...
        public BindableTypeNew? ComponentType
        {
            get
            {
                if (this.property is null)
        ...
        public BindableTypeNew PropertyType
        {
            get
            {
                if (this.property is null)
        ...
        public PropertyFilterNew? Filter
        {
            get => this.filter;
        ...
        private void Update()
        {
            ...
            if (SnoopModesNew.MultipleDispatcherMode
                && d is not null
        ...
        public void UpdateBindingError()
        {
            ...
            using (new ScopeGuardNew(() => this.ignoreUpdate = true, () => this.ignoreUpdate = false).Guard())
            {
                BindingDiagnosticHelperNew.Instance.TrySetBindingError(expression, d, dp, s => this.BindingError = s);
            }
        }

        public static List<PropertyInformationNew> GetProperties(object? obj)
        {
            return GetProperties(obj, PertinentPropertyFilterNew.Filter);
        }

        public static List<PropertyInformationNew> GetProperties(object? obj, Func<object, PropertyDescriptor, bool> filter)
        {
            var properties = new List<PropertyInformationNew>();
            ...
            foreach (var property in propertyDescriptors)
            {
                if (filter(obj, property))
                {
                    try
                    {
                        var prop = new PropertyInformationNew(obj, property, property.Name, property.DisplayName);
                        properties.Add(prop);
                    }
                    catch (Exception e)
                    {
                        LogHelperNew.WriteError($"Failed to create PropertyInformation for property '{property.Name}' on '{obj}'.{Environment.NewLine}{e}");
                    }
                }
            }
            ...
            if (obj is ResourceDictionary resourceDictionary) // if the object is a ResourceDictionary, add the items in the collection as properties
            {
                foreach (var key in resourceDictionary.Keys)
                {
                    ...
                    {
                        var value = item;
                        var info = new PropertyInformationNew(value ?? key!,
                            resourceDictionary, "this[" + key + "]", value)
                        {
            ...
            else if (obj is ICollection collection) // if the object is a collection, add the items in the collection as properties
            {
                ...
                foreach (var item in collection)
                {
                    ...
                    var info = new PropertyInformationNew(item, collection, "this[" + index + "]", item)
                    {
                        IsCollectionEntry = true,
        ...
        /// </summary>
        /// <returns></returns>
        private static IList<PropertyInformationNew>? GetExtendedProperties(object? obj)
        {
            if (obj is null)
            {
                return null;
            ...
            {
                var key = ResourceKeyCacheNew.Instance.GetKey(obj);

                if (ResourceKeyHelperNew.IsValidResourceKey(key))
                {
                    var prop = new PropertyInformationNew(key!, null, "x:Key", key!, isCopyable: true);
                    return new List<PropertyInformationNew>
                {
                    prop
                };
                }
            ...
            {
                return new List<PropertyInformationNew> { new(obj, null, "ToString", obj, isCopyable: true) };
            }

            if (obj is AutomationPeer automationPeer)
            {
                var automationProperties = new List<PropertyInformationNew>
            {
                ...
                automationProperties.Add(new PropertyInformationNew(obj, null, "SupportedPatterns", string.Join(", ", supportedPatterns), isCopyable: true));
        ...
        {
            var other = obj as PropertyInformationNew;
